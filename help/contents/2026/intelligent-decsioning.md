---
title: 透過智慧型決策提供即時客戶體驗
description: 瞭解如何使用Adobe Journey Optimizer進行智慧型決策，並即時提供個人化客戶體驗。
role: Admin, Developer, Leader, User
level: Beginner, Intermediate
doc-type: Event
duration: 2269
last-substantial-update: 2026-05-08T00:00:00Z
jira: KT-21144
hide: true
hidefromtoc: true
source-git-commit: e54140d809f316b0f9b87f765a3f147a40ec9284
workflow-type: tm+mt
source-wordcount: '175'
ht-degree: 0%

---


# 透過智慧型決策提供即時客戶體驗

即時提供個人化客戶體驗不只需要客戶資料。 它仰賴智慧決策，評估多個選項並在適當時機提供最相關的選項。 使用Adobe Journey Optimizer，品牌可以建構體驗、套用決定邏輯，以及在客戶歷程中擴展個人化。

>[!VIDEO](https://video.tv.adobe.com/v/3486516/?captions=chi_hant&learn=on&enablevpops)

本課程說明智慧型決策的重要性、運作方式，以及相關的業務優點。 瞭解目錄結構描述和選件等基本結構如何簡化縮放，而決策規則和AI模型等工具如何最佳化個人化傳遞。 探索以動態方式排名優惠的方法，包括使用AI驅動的模型來進行個人化最佳化。

組織可以達成具影響力的結果，例如提高客戶參與度、改善轉換率，以及跨管道的順暢擴充能力。 以下內嵌的影片提供這些概念的詳細逐步解說，並示範Adobe Journey Optimizer如何協助進行智慧決策。


